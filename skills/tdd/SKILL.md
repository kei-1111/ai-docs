...
- The change adds or modifies logic in a testable layer — the project's TDD rule
  ...
  data access, and serialization are typical.
...
1. **Test list** — from the requirement, write the list of expected behaviors including edge
   cases, implementing none of them.
2. **Red** — turn exactly ONE list item into a test (conventions per the suite rule for the
   ...
   does not.
3. **Green** — write the minimum production code that makes the new test pass; re-run the
   same task.
