...
These rules govern every written output — rules, skills, documents, Issue and PR bodies, reports:

- Concise: one clear statement over repeated wording, exhaustive safeguards, or speculative edge cases.
- Readable with no prior context: a first-time reader needs no access to the discussion, session, or superseded version that produced the text.
- Present state only: describe what is or what to do — never the path there (no "previously" / "was" / "now", no reactions to earlier versions, no deliberation or process narration).
- Scannable: short bullets or tables over dense prose where the content allows.
